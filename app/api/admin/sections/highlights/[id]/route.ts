...
import { getHighlightById, updateHighlight, deleteHighlight } from "@/lib/db/queries/highlights";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const row = await updateHighlight(Number(id), body);
  if (!row) return Response.json({ error: "Introuvable" }, { status: 404 });
  revalidatePath("/");
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const row = await updateHighlight(Number(id), { status });
  if (!row) return Response.json({ error: "Introuvable" }, { status: 404 });
  revalidatePath("/");
  revalidatePath("/admin");
  return Response.json(row);
